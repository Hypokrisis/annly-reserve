...
const steps = [
            ...
            <div className="bg-white rounded-2xl border-2 border-space-border p-6 shadow-sm">
                ...
                <div className="space-y-3">
                    {[{ label: 'Nombre del Negocio', value: 'Barbería El Rey' }, { label: 'Ciudad', value: 'San Juan, PR' }, { label: 'Tu URL', value: 'spaceyreserve.app/barberiaelrey' }].map((f, i) => (
                        <div key={i}>
                            <p className="text-[9px] font-black text-space-muted uppercase tracking-widest mb-1">{f.label}</p>
                            <div className="h-10 bg-space-bg border border-space-border rounded-xl flex items-center px-4">
                                <span className="text-sm font-semibold text-space-text">{f.value}</span>
                            </div>
                        </div>
                    ))}
                ...
                <div className="space-y-2">
                    {[{ name: 'Corte + Barba', duration: '45 min', price: '$20' }, { name: 'Solo Corte', duration: '30 min', price: '$15' }, { name: 'Diseño Líneas', duration: '20 min', price: '$12' }].map((s, i) => (
                        <div key={i} className="flex items-center justify-between p-3 bg-space-bg rounded-xl border border-space-border">
                            <div>
                                <p className="text-xs font-black text-space-text">{s.name}</p>
                                <p className="text-[9px] text-space-muted font-bold">{s.duration}</p>
                            </div>
                            <span className="text-sm font-black text-space-primary">{s.price}</span>
                        </div>
                    ))}
    ...
    },
    {
        ...
        desc: 'Cuando alguien reserva, el cliente recibe confirmación por WhatsApp al instante. El día anterior recibe un recordatorio. Tú no haces nada.',
        icon: MessageCircle,
        demo: (
            <div className="bg-[#0B141A] rounded-2xl p-4 shadow-sm">
                ...
                <div className="space-y-2">
                    ...
                    <div className="bg-[#1F2C34] rounded-2xl rounded-tl-none p-3 max-w-[85%]">
                        <p className="text-white text-[11px] leading-relaxed">Puedes ver tu cita aquí: <span className="text-blue-400 underline">spaceyreserve.app/book/rey</span></p>
                        <p className="text-[9px] text-white/40 mt-1 text-right">10:05 AM ✓✓</p>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-white rounded-2xl border-2 border-space-border p-6 shadow-sm">
                ...
                <div className="bg-[#E3FFCA] rounded-2xl rounded-tr-none p-4 border border-[#C5EB9E] mb-4">
                    <p className="text-[12px] text-[#111b21] leading-relaxed">
                        ¡Hola <strong>Carlos</strong>! Soy el staff de <strong>Barbería El Rey</strong>. Te tenemos: <em>"Hoy 2x1 en cortes de barba, hasta las 8pm 🔥"</em>. Reserva: <span className="text-blue-500 underline">spaceyreserve.app/book/rey</span>
                    </p>
                </div>
...
const testimonials = [
    ...
    { name: 'Carlos R.', biz: 'The Barber Room, NYC', quote: 'En una semana ya tenía mi página de reservas funcionando. Mis clientes volvieron con la primera campaña de marketing.', stars: 5 },
    { name: 'Yanira M.', biz: 'Salón Luxury Nails, Miami', quote: 'El recordatorio automático de WhatsApp eliminó casi todas las no-shows. La mejor inversión para mi negocio.', stars: 5 },
];
        ...
        <div className="min-h-screen bg-space-bg text-space-text font-sans">
            ...
            <section className="max-w-6xl mx-auto px-6 pb-24">
                <div className="flex flex-col lg:flex-row gap-8">
                    ...
                    <div className="flex-1 bg-white rounded-3xl border-2 border-space-border p-8 shadow-sm">
                        ...
                        <div className="max-w-md">{steps[activeStep].demo}</div>
                        <div className="flex items-center justify-between mt-6 pt-6 border-t border-space-border">
                            <button onClick={() => setActiveStep(Math.max(0, activeStep - 1))} disabled={activeStep === 0} className="text-sm font-bold text-space-muted hover:text-space-text transition disabled:opacity-30 flex items-center gap-2">
                                ← Anterior
                            </button>
                            {activeStep < steps.length - 1 ? (
                                <button onClick={() => setActiveStep(activeStep + 1)} className="flex items-center gap-2 bg-space-primary text-white font-black text-xs uppercase tracking-widest px-6 py-3 rounded-xl hover:bg-space-primary/90 transition-all active:scale-95">
                                    Siguiente Paso <ArrowRight size={14} />
                                </button>
                            ) : (
                                <Link to="/signup" className="flex items-center gap-2 bg-space-primary text-white font-black text-xs uppercase tracking-widest px-6 py-3 rounded-xl hover:bg-space-primary/90 transition-all active:scale-95">
                ...
                <div className="max-w-5xl mx-auto px-6">
                    <div className="flex flex-wrap gap-3 justify-center">
                        {['✅ Reservas 24/7', '📲 WhatsApp Automático', '📣 Marketing Masivo', '👥 Multi-Empleado', '📅 Calendario en Tiempo Real', '🔒 100% Seguro', '⚡ Setup en 5 Minutos', '💳 Cobros Online (Premium)', '📊 Estadísticas Completas', '🌐 Página Web Incluida'].map((pill, i) => (
                            <span key={i} className="px-4 py-2 bg-space-bg border border-space-border rounded-full text-sm font-bold text-space-text">{pill}</span>
                        ))}
                    </div>
                </div>
            </section>
            ...
            <section className="bg-space-text text-white py-20">
                <div className="max-w-2xl mx-auto text-center px-6">
                    ...
                    <h2 className="text-4xl font-black uppercase tracking-tight mb-4">Tu negocio merece <span className="text-space-primary">más clientes.</span></h2>
                    ...
                    <div className="flex items-center justify-center gap-4 flex-wrap">
                        ...
                        <Link to="/pricing" className="text-white/60 hover:text-white text-sm font-bold transition">Ver precios →</Link>
                    </div>
                </div>
            </section>
        </div>
